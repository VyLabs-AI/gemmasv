...
map -- alpha = SVDD(K) with the implicit-gradient backward pass that reuses the
bordered KKT inverse R -- but obtains the active-set partition and R from the
...
Backward (identical to `diff_svdd._SVDDFn`, theory doc sec 5.2): for a fixed
